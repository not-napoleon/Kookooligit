...
int render_map_window(InfiniteMap *map, MapGraphicsState *mgs,
                      Rect *map_window) {
    ...
    for (i = 0; i <= map_window_x_chars; i++) {
        sprite_grid[i] = (struct Drawable *)malloc(
            sizeof(struct Drawable) * map_window_y_chars + 1);
    }
    get_tile_grid(map, map_window_x_chars, map_window_y_chars, mgs->draw_cursor,
                  sprite_grid);
