...
bool tiles_initilized() { return _was_initilized; }
...
void init_tile_types() {
    tile_data[OffGrid] =
        _make_tile_type(OffGrid_sprite, "off_grid", "Unexplored space", true);
    tile_data[OpenSpace] =
        _make_tile_type(OpenSpace_sprite, "open", "An unobstructed corridor "
                                                  "of some kind",
                        true);
    tile_data[ImpassableWall] =
        _make_tile_type(ImpassableWall_sprite, "wall",
                        "A hardened bulkhead, "
                        "probably with hardened vaccuum on the other side",
                        false);
...
Tile *get_default_tile() { return &default_tile; }
