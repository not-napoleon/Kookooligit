...
void draw_sprite_at_point(const struct Drawable *drawable, const int x,
    ...
    /* x and y coordinates are given in tiles; the conversion to pixels happens
     * here.*/
    if (!_was_initilized) {
    ...
    if (drawable->draw_cursor == 1) {
        ...
        SDL_SetRenderDrawColor(get_main_renderer(), cursor_bg_color.r,
                               cursor_bg_color.g, cursor_bg_color.b,
                               SDL_ALPHA_OPAQUE);
        SDL_RenderFillRect(get_main_renderer(), &write_coords);
        SDL_SetRenderDrawColor(get_main_renderer(), map_bg_color.r,
                               map_bg_color.g, map_bg_color.b,
                               SDL_ALPHA_OPAQUE);
    }
    ...
    SDL_RenderCopy(get_main_renderer(), sprite_cache[drawable->sprite_id], NULL,
                   &write_coords);
}

SDL_Texture *load_sprite_from_font(const char *glyph, const Color fg) {
    SDL_Surface *tmp =
        TTF_RenderText_Solid(sprite_font, glyph, convert_color(fg));
    SDL_Texture *retval =
        SDL_CreateTextureFromSurface(get_main_renderer(), tmp);
    SDL_FreeSurface(tmp);
    return retval;
}
...
void init_sprites_from_font(const char *font_path, const int point_size) {
    ...
    sprite_cache[BattleBot_sprite] = load_sprite_from_font("&", color_lit);
...
int get_char_width(const int pixel_width) { return pixel_width / at_width; }
