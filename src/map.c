...
void dump_edge_parameters(InfiniteMap *map) {
    ...
    for (i = 0; i < MAP_SECTION_BUFFER; i++) {
        ...
        DEBUG("Section %d top:    x1=%d, w1=%d;\tx2=%d, w2=%d;\tx3=%d, w3=%d\n",
              i, sec->top_x_positions[0], sec->top_widths[0],
        ...
        DEBUG("Section %d bottom: x1=%d, w1=%d;\tx2=%d, w2=%d;\tx3=%d, w3=%d\n",
              i, sec->bottom_x_positions[0], sec->bottom_widths[0],
...
Tile *get_tile(const InfiniteMap *map, int x, int y) {
    ...
    if (y < 0) {
        int section_index =
            roto_indx((map->current_section - 1), MAP_SECTION_BUFFER);
...
int generate_initial_map(InfiniteMap *map) {
    ...
    for (i = 1; i < MAP_SECTION_BUFFER; i++) {
        ...
        generate_map_section(map->section_list[i],
...
int get_tile_grid(InfiniteMap *map, const int window_x_chars,
    ...
    for (x = x_start; x <= x_end; x++) {
        for (y = y_start; y <= y_end; y++) {
            ...
            if (x == map->at_location.x && y == map->at_location.y) {
                /* Draw player case */
            ...
            if (draw_cursor && x == map->cursor_location.x &&
...
int calculate_visible_tiles(InfiniteMap *map, Point at_location) {
    ...
    dark_map(map);
    /* make sure the tile the player is on is lit.  Player could land on an
     * unlit
    ...
    fov_circle(fov_settings, map, NULL, at_location.x, at_location.y,
               VISION_RADIUS);
...
void rotate_buffers(InfiniteMap *map, bool pos_y_dir) {
    ...
    if (!pos_y_dir) {
        int to_discard =
            roto_indx((map->current_section + offset), MAP_SECTION_BUFFER);
        int edge =
            roto_indx((map->current_section - offset), MAP_SECTION_BUFFER);
        map->current_section =
            roto_indx((map->current_section - 1), MAP_SECTION_BUFFER);
